...
본 프로젝트는 STM32 M4를 기반으로 L293로 DC모터를 제어하도록 구현을 목표로 한다.


### 2. 개발 환경 및 스펙 (MCU, IDE, 도구)
| 구분 | 항목 | 내용/스펙 | 비고 |
| :--- | :--- | :--- | :--- |
| HW | MCU | STM32(ARM Cortex-M4) | |
| HW | IC | L293 | |
| HW | Actuator | DC모터| |
| SW | Language | C | |
| SW | IDLE | VScode | |
...
### 3-1. 기능 요구사항
- M4 버튼 눌렀을때 모터가 작동해야한다
    - 단, 전원 인가 전에는 모터는 정지해야한다 
- 다시 버튼을 눌렀을때 모터의 방향은 반대로 돌아야한다
    - 키를 3초 이전에 해제하면 회전 방향을 변경해야 한다.
    - 키를 3초 이상 누르면 모터를 정지해야 한다.
    - 단, 바로 방향을 바꾸면 탈조의 위험이 있기 때문에 잠시 정지했다가 동작해야한다

- UART 통신으로 `S/s` 명령을 입력하면 모터를 정지해야 한다.
- UART 통신으로 `F/f` 명령을 입력하면 모터는 정방향(`CW`)으로 동작해야한다
- UART 통신으로 `S/s` 명령을 입력하면 모터는 역방향(`CCW`)으로 동작해야한다
- UART통신 입력창에 `1~9`까지 숫자를 입력하면 모터의 속도를 변경 할 수 있다 
- 정의되지 않은 UART 명령은 무시 한다.

### 3-2. 비기능 요구사항 / 제약사항
- L293의 모터 전원은 반드시 CP2102 모듈의 `+3.3V` 또는`+5V`를 이용하여 연결한다
- PWM 주파수 및 PWM Duty Rate 설정은 실험에 의하여 적절한 값을 적용하도록 한다
    - 단, PWM의 Duty Rate이 너무 적으면 모터가 구동되지 않으므로 
    Duty 범위는 `50% ~ 100%`로 한다


## 시스템 아키텍처

### 4-1. 시스템 동작 흐름도 (Flowchart)
![플로우차트](./images/Flowchart.png)
### 4-2. 상태 전이도 (State Machine Diagram)
![FSM](./images/FSM.png)
